...
MANIFEST_SHA = "a" * 64
KNOWN_SCENARIOS = frozenset({"location-change", "uptime-reset"})


def state(phase, scenario_id=None, error_code=None):
    if phase in {"failed", "manual_recovery_required"} and error_code is None:
...
class ScenarioStateTests(unittest.TestCase):
    def test_allows_only_the_frozen_lifecycle_edges(self):
        ...
        for current, action, expected in allowed:
            ...
            with self.subTest(current=current, action=action):
                result = transition(
                    state(current, scenario_id),
                    action,
                    supplied_id,
                    known_scenario_ids=KNOWN_SCENARIOS,
                )
    ...
    def test_baseline_reset_is_a_safe_noop(self):
        ...
        self.assertIs(transition(current, "reset", known_scenario_ids=KNOWN_SCENARIOS), current)
    ...
    def test_status_is_a_safe_noop_in_every_phase(self):
        for phase in ScenarioPhase:
            ...
            with self.subTest(phase=phase):
                self.assertIs(transition(current, "status", known_scenario_ids=KNOWN_SCENARIOS), current)
        ...
        for action in ("apply", "observe"):
            for scenario_id in (None, "", "UPPER CASE", "../escape"):
                with self.subTest(action=action, scenario_id=scenario_id), self.assert_transition_error(
                    ...
                    transition(state("baseline"), action, scenario_id, known_scenario_ids=KNOWN_SCENARIOS)

    def test_start_and_persisted_state_require_manifest_membership(self):
        with self.assert_transition_error("unknown_scenario"):
            transition(
                state("baseline"),
                "apply",
                "not-in-manifest",
        ...
        for action in ("apply", "poll", "observe", "ai_check", "reset"):
            with self.subTest(action=action), self.assert_transition_error("manual_recovery_required"):
                transition(
                    current,
                    action,
                    "location-change" if action == "apply" else None,
                    known_scenario_ids=KNOWN_SCENARIOS,
                )
    ...
    def test_invalid_order_returns_stable_error(self):
        for current, action in (
            ...
            with self.subTest(current=current, action=action), self.assert_transition_error(
                ...
                transition(
                    state(current, None if current in {"baseline", "reset"} else "location-change"),
                    action,
                    known_scenario_ids=KNOWN_SCENARIOS,
                )
    ...
    def test_failure_and_recovery_helpers_require_stable_error_codes(self):
        ...
        recovery = recovery_required(
            state("failed", "location-change", "runner_timeout"),
            "reset_failed",
            known_scenario_ids=KNOWN_SCENARIOS,
        )
        ...
        for helper in (failed, recovery_required):
            with self.subTest(helper=helper.__name__), self.assert_transition_error("invalid_error_code"):
                helper(state("applied", "location-change"), "", known_scenario_ids=KNOWN_SCENARIOS)

    def test_state_constructor_rejects_impossible_persisted_combinations(self):
        for phase, scenario_id, manifest_sha, error_code in (
            (ScenarioPhase.BASELINE, "location-change", MANIFEST_SHA, None),
            (ScenarioPhase.APPLIED, None, MANIFEST_SHA, None),
            (ScenarioPhase.APPLIED, "location-change", "bad-sha", None),
            (ScenarioPhase.APPLIED, "location-change", None, None),
            (ScenarioPhase.FAILED, "location-change", MANIFEST_SHA, None),
            (ScenarioPhase.OBSERVED, "location-change", MANIFEST_SHA, "unexpected_error"),
        ):
            with self.subTest(phase=phase, scenario_id=scenario_id), self.assertRaisesRegex(
                ValueError, "invalid_state"
            ):
                ScenarioState(phase, scenario_id, manifest_sha, error_code)
