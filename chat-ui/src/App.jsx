...
export default function App({ chatStore = defaultStore, identity = defaultIdentity, api = apiClient }) {
  ...
  const runDemo = async (scenario) => { if (demoActiveRef.current || !demoTargetId) return; demoActiveRef.current = true; setDemoRunning(scenario.label); setDemoResult(null); setDemoExpectation(null); setDemoError(""); try { const result = await api.runDemoScenario(scenario.id, demoTargetId, token); setDemoResult({ ...result, title: scenario.label }); } catch (error) { if (error.status === 401) expireSession(); else setDemoError("Senaryo tamamlanamadı."); } finally { demoActiveRef.current = false; setDemoRunning(""); } };
  const resetDemo = async () => { if (demoActiveRef.current || !demoTargetId) return; demoActiveRef.current = true; setDemoRunning("Laboratuvar sıfırlama"); setDemoResult(null); setDemoExpectation(null); setDemoError(""); try { const result = await api.resetDemo(demoTargetId, token); setDemoResult({ ...result, title: "Laboratuvar sıfırlandı" }); } catch (error) { if (error.status === 401) expireSession(); else setDemoError("Laboratuvar sıfırlanamadı."); } finally { demoActiveRef.current = false; setDemoRunning(""); } };
  const askDemoQuestion = async (question, expectation) => { if (demoQuestionActiveRef.current || !expectation) return; demoQuestionActiveRef.current = true; setDemoQuestionRunning(true); try { await send(question, expectation); } finally { demoQuestionActiveRef.current = false; setDemoQuestionRunning(false); } };
      ...
      <div className={styles.shell} data-sidebar-collapsed={sidebarCollapsed || undefined}>
        ...
        <main ref={mainRef} tabIndex="-1" id="investigation-main" className={styles.main}>
          <header className={styles.header}>
            ...
            {demoMode?.enabled && demoScenarios.length ? <button type="button" className={styles.demoEntry} onClick={() => setDemoOpen(true)}>Demo Kontrolleri</button> : null}
        ...
        <DemoControls open={demoOpen} scenarios={demoScenarios} targets={demoTargets} selectedTargetId={demoTargetId} runningLabel={demoRunning} result={demoResult} verification={demoVerification} questionRunning={demoQuestionRunning} error={demoError} onClose={() => setDemoOpen(false)} onTargetChange={(targetId) => { setDemoTargetId(targetId); setDemoResult(null); setDemoExpectation(null); setDemoError(""); }} onRun={runDemo} onReset={resetDemo} onAsk={askDemoQuestion} />
      </div>
