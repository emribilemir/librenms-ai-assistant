...
export default function App({ chatStore = defaultStore, identity = defaultIdentity, api = apiClient }) {
  ...
  const [deleteTarget, setDeleteTarget] = useState(null); const [sessionExpired, setSessionExpired] = useState(false); const [suggestions, setSuggestions] = useState([]); const [suggestionsUnavailable, setSuggestionsUnavailable] = useState(false);
  ...
  const selectedMessages = state.selectedThreadId ? state.messages[state.selectedThreadId] || [] : []; const run = state.selectedThreadId ? state.runs[state.selectedThreadId] : null; const runtime = useLibreNmsExternalStoreRuntime(chatStore, state.selectedThreadId, send, () => activeRunsRef.current.get(state.selectedThreadId)?.controller.abort(), suggestions);
  return <AssistantRuntimeProvider runtime={runtime}><div className={styles.shell}><a className={styles.skip} href="#investigation-main">Skip saved investigations</a><ThreadDrawer open={state.drawerOpen} onClose={() => chatStore.dispatch({ type: "drawer.close" })} onCreate={create}><ThreadList threads={state.threads} selectedThreadId={state.selectedThreadId} onSelect={select} onDelete={(thread, trigger) => { deleteTriggerRef.current = trigger; setDeleteTarget(thread); }} /></ThreadDrawer><main ref={mainRef} tabIndex="-1" id="investigation-main" className={styles.main}><header className={styles.header}><button type="button" className={styles.menu} onClick={() => chatStore.dispatch({ type: "drawer.open" })} aria-label="Open investigations">☰</button><div><p>LibreNMS · Read-only assistant</p><h1>{state.threads.find((thread) => thread.id === state.selectedThreadId)?.title || "AI Assistant"}</h1></div><span className={styles.status}>Connected to live pipeline</span></header>{sessionExpired ? <p className={styles.error} role="alert">Your LibreNMS session has expired. Refresh the page to continue.</p> : run?.error && <p className={styles.error} role="alert">{run.error.message}</p>}<RunProgress run={run} /><AssistantThread messages={selectedMessages} running={run?.status === "running"} onSend={send} onCancel={() => activeRunsRef.current.get(state.selectedThreadId)?.controller.abort()} canRetry={!sessionExpired && run?.canRetry} onRetry={() => { const last = [...selectedMessages].reverse().find((message) => message.role === "user"); if (last) send(last.content); }} suggestionsUnavailable={suggestionsUnavailable} /><RunMetrics metrics={run?.metrics} /></main><DeleteThreadDialog open={Boolean(deleteTarget)} threadTitle={deleteTarget?.title} onCancel={closeDelete} onConfirm={confirmDelete} /></div></AssistantRuntimeProvider>;
}
