...
export default function App({ chatStore = defaultStore, identity = defaultIdentity, api = apiClient }) {
  ...
  const create = async () => { if (isCurrentThreadPristine()) return; replaceQueueContext(); try { const thread = await api.createThread(token); chatStore.dispatch({ type: "thread.created", thread }); } catch (error) { if (error.status === 401) expireSession(); } };
