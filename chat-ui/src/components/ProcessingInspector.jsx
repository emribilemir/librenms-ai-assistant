...
export function ProcessingInspector({ value, embedded = false }) {
  ...
  const selectTab = (nextTab) => {
    setTab(nextTab);
    ({ summary: summaryTabRef, json: jsonTabRef })[nextTab].current?.focus();
  };
  ...
  const content = (
    <>
      ...
      <div id={tabPanelId} role="tabpanel" aria-labelledby={{ summary: summaryTabId, json: jsonTabId }[tab]}>
        {tab === "summary" ? <Summary inspection={inspection} /> : (
          <pre className={`${styles.code} ${styles.json}`}>{JSON.stringify(inspection, null, 2)}</pre>
        )}
      </div>
    </>
  );
