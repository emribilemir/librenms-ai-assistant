...
Layers (single-device resolve_device), highest confidence first:
    ...
    3. device alias         (device-level alias only)
    4. exact catalog ID     (SKU/model/canonical_name/catalog alias/search_text)
    5. typo                 (edit distance <= 1 + keyboard/prefix/suffix scoring)
    6. broader metadata     (brand/sku/model/canonical_name/search_text tokens)
    7. ambiguous / no_match

Exact identity evidence always outranks fuzzy evidence. Catalog aliases are
metadata identities, not device aliases.
...
_CLITIC_RE = re.compile(r"[’'][^\W_]{0,8}$")
...
def _is_adjacent(a, b):
    return (b in QWERTY_ADJACENT.get(a, set()) or
            a in QWERTY_ADJACENT.get(b, set()))


def _metadata_tok_match(qt, mt):
    """Conservative token match for model metadata."""
    if mt == qt:
        return True
    if qt.isdigit() and mt.startswith(qt) and any(ch.isalpha() for ch in mt):
        return True
    if min(len(qt), len(mt)) < MIN_METADATA_PREFIX_LEN:
        return False
    return mt.startswith(qt) or qt.startswith(mt)


def _hostname_group_tok_match(qt, ht):
    """Strict token equality for hostname-group filtering."""
    return qt == ht
...
def _literal_exact_metadata_models(q_raw, idx):
    """Exact catalog identity using literal case-insensitive text equality.

    Used by the device-set path so a broad phrase like ``2530 48G`` does not
    collapse into the literal model ``2530-48G`` after separator removal, while
    an explicitly written model such as ``2530-8-PoEP`` remains authoritative.
    """
    q = str(q_raw or "").strip().lower()
    if not q:
        return []
    matched=[]
    for m in idx["models"]:
        identifiers = {
            str(m.get("sku", "")).strip().lower(),
            str(m.get("model", "")).strip().lower(),
            str(m.get("canonical_name", "")).strip().lower(),
            str(m.get("search_text", "")).strip().lower(),
        }
        identifiers |= {str(a).strip().lower() for a in (m.get("aliases", []) or [])}
        identifiers.discard("")
        if q in identifiers:
            matched.append(m)
    return matched
...
def _exact_metadata_models(q_raw, idx):
    """Exact authoritative catalog identifiers, excluding brand-only matches."""
    q = normalize(q_raw)
    if not q:
        return []
    matched = []
    for m in idx["models"]:
        identifiers = {
            ...
            normalize(m.get("search_text", "")),
        }
        identifiers |= {normalize(a) for a in (m.get("aliases", []) or [])}
        identifiers.discard("")
        if q in identifiers:
            matched.append(m)
    return matched
...
def _metadata_match_models(q_raw, idx):
    ...
    for m in idx["models"]:
        ...
        if all(any(_metadata_tok_match(t, mt) for mt in mtoks) for t in toks):
            matched.append(m)
    return matched


def _hostname_group_matches(device, toks):
    if not toks or not any(any(ch.isalpha() for ch in t) for t in toks):
        return False
    htoks = _hostname_tokens(device.get("hostname", ""))
    return all(any(_hostname_group_tok_match(t, ht) for ht in htoks) for t in toks)
...
def _matched_fields(q_raw, matched_models, group_devices):
    ...
    for m in matched_models:
        ...
        if any(q == normalize(a) for a in (m.get("aliases", []) or [])):
            fields.add("alias")
        brand_tokens = _TOKEN_RE.findall(_strip_clitic(str(m.get("brand", "")).lower()))
        if any(any(_metadata_tok_match(t, bt) for bt in brand_tokens) for t in toks):
            fields.add("brand")
...
def resolve_device(query, inventory=None):
    ...
    # 4) exact catalog identity before fuzzy hostname matching
    exact_models = _exact_metadata_models(q_raw, idx)
    if exact_models:
        devs = _devices_for_models(exact_models, devices)
        if len(devs) == 1:
            return _resolved(devs[0], "metadata", query=q_raw, models=exact_models)
        if len(devs) > 1:
    ...
    # 6) broader metadata single-device
    models = _metadata_match_models(q_raw, idx)
    if models:
        devs = _devices_for_models(models, devices)
        if len(devs) == 1:
...
def _dedupe(devices):
    ...
    for d in devices:
        ...
        key = ("device_id", str(device_id)) if device_id is not None else ("hostname", normalize(d.get("hostname", "")))
        if key not in seen:
            seen.add(key)
            out.append(d)
    return out
...
def resolve_device_set(query, inventory=None):
    ...
    all_devices = _dedupe(model_devices + direct_devices + group_devices)
